...
GraphSvc.prototype.deleteConnection = function(baseEntityIndexName, baseEntityKeyValue, connectedEntityIndexName, connectedEntityKeyValue, relationshipName, options) {
	...
	return (function() {
		if(options.runBefores) {
			return RunBeforeProcessing('delete', baseEntityKeyValue, options.graphRequest);
		} else {
			return Q.fcall(function() { return data; });
		}
	})().then(function(data) {
		...
		if(options.runAfters) {
			return RunAfterProcessing('delete', resp, options.graphRequest);
		} else {
			return Q.fcall(function() { return resp; });
		}
	});
...
GraphSvc.prototype.createConnection = function(baseEntityIndexName, baseEntityKeyValue, connEntityIndexName, connEntityData, relName, dir, options) {
	...
	return (function() {
		if(options.runBefores) {
			return RunBeforeProcessing('connect', {
				baseEntity: {
					index: baseEntityIndexName,
					key: baseEntityKeyValue
				},
				connectedEntity: {
					index: connEntityIndexName,
					data: connEntityData
				}
...
function RunProcessing(before, operation, response) {
	...
	if(!graphReq.connection) { // entity processing
		...
		process = _.filter(processStore, function(itm) {
			return (itm.operation == operation && itm.path == graphReq.inboundPath) || 
				   (itm.operation == operation && itm.path == graphReq.outboundPath) || 
			       (itm.operation == operation && itm.path == '*');
		});
	}
