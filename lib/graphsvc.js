...
GraphSvc.prototype.getRelatedNodes = function(nodeKeyValue, nodeIndex, relationshipType, dir, options) {
	var $this = this;
			...
			getRelatedNodeCallParams.forEach(function (connObj) {
				result = result.then(function() {
					if(connObj.isCustomProperty) {
						...
						return $this.getRelatedNodes(keyValue, indexName, connObj.relationshipName, connObj.dir, {
								skip: options.skip, 
								limit: options.limit
							})
...
GraphSvc.prototype.deleteConnection = function(baseEntityIndexName, baseEntityKeyValue, connectedEntityIndexName, connectedEntityKeyValue, relationshipName, options) {
	...
	return (function() {
		...
		if(options.runAfters) {
			return RunAfterProcessing('delete', resp, options);
		} else {
			return Q.fcall(function() { return resp; });
		}
	});
...
GraphSvc.prototype.createConnection = function(baseEntityIndexName, baseEntityKeyValue, connEntityIndexName, connEntityData, relName, dir, options) {
	var $this = this;
	if(!options) options = {};
	options.runBefores = (options.runBefores == false ? false : true);;
	options.runAfters = (options.runAfters == false ? false : true);;
	...
	return (function() {
		if(options.runBefores) {
			return RunBeforeProcessing('create', null, options);
		} else {
			return Q.fcall(function() { return null; });
		}
	})().then(function(data) {
		...
		if(options.runAfters) {
			return RunAfterProcessing('create', resp, options);
		} else {
			return Q.fcall(function() { return resp; });
		}
	});
