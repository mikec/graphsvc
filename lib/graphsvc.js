...
GraphSvc.prototype.addConnection = function(a, b, c) {
	...
	this.post('/' + connection.start.index + '/:keyvalue/' + connection.start.connection, relationshipCreateHandler);
	this.get('/' + connection.start.index + '/:keyvalue/' + connection.start.connection, relationshipGetHandler);
	this.delete('/' + connection.start.index + '/:keyvalue/' + connection.start.connection + '/:connectionkeyvalue', relationshipDeleteHandler);
	
	//routes for end entity
	this.post('/' + connection.end.index + '/:keyvalue/' + connection.end.connection, relationshipCreateHandler);
	this.get('/' + connection.end.index + '/:keyvalue/' + connection.end.connection, relationshipGetHandler);
	this.delete('/' + connection.end.index + '/:keyvalue/' + connection.end.connection + '/:connectionkeyvalue', relationshipDeleteHandler);
...
GraphSvc.prototype.getEntity = function(keyValue, indexName, options) {
	...
	return (function() {
		if(options.runBefores) {
			return RunBeforeProcessing('read', null, options.graphRequest);
		} else {
			...
			getRelatedNodeCallParams.forEach(function (connObj) {
				result = result.then(function() {
					if(connObj.isCustomProperty) {
						return connObj.fn(keyValue, options).then(function(data) {
							nodeData[connObj.property] = data;
							return Q.fcall(function() { return nodeData; });
						});
					} else {
						return $this.getRelatedNodes(keyValue, indexName, connObj.relationshipName, connObj.dir, options.skip, options.limit)
...
function nodeUpdateHandler(req, res) {
	...
	__app.updateEntity(req.body, graphReq.index, true, graphReq).then(function(r) {
		res.send(r);
	}, function(err) {
...
GraphSvc.prototype.updateEntity = function(data, indexName, runBefores, runAfters, graphRequest) {	
	var $this = this;
	return (function() {		
		if(runBefores) {
			return RunBeforeProcessing('update', data, graphRequest);
		} else {
		...
		var nodeData = (r.body && r.body[0] && r.body[0].data ? r.body[0].data : {});
		if(runAfters) {
			return RunAfterProcessing('update', nodeData, graphRequest);
		} else {
			return Q.fcall(function() { return data; });
		}
	});
...
GraphSvc.prototype.deleteEntity = function(keyValue, indexName, options) {
	...
	return (function() {
		if(options.runBefores) {
			return RunBeforeProcessing('delete', null, options);
		} else {
			return Q.fcall(function() { return null; });
		}
	})().then(function() {
		...
		if(options.runAfters) {
			return RunAfterProcessing('delete', r, options);
		} else {
			return Q.fcall(function() { return null; });
		}
	})
...
function nodeCreateHandler(req, res) {
	...
	__app.createEntity(req.body, graphReq.index, true, true, graphReq).then(function(r) {
		res.send(r);
	}, function(err) {
		//respond with an error
		res.statusCode = 500;
...
GraphSvc.prototype.createEntity = function(data, indexName, runBefores, runAfters, graphRequest) {
	var $this = this;
	return (function() {		
		if(runBefores) {
			return RunBeforeProcessing('create', data, graphRequest);
		} else {
			return Q.fcall(function() { return data; });
		}
	})().then(function(data) {
		return $this.createIndexedNode(data, indexName).then(function(resp) {
			...
			return (function() {		
				if(runAfters) {
					return RunAfterProcessing('create', d, graphRequest);
				} else {
...
GraphSvc.prototype.getConnectedEntities = function(baseEntityKeyValue, baseEntityIndexName, connectionName, options) {
	...
	//TODO: get 'entityName.connectionName' ('bands.followers')
	...
	var direction;
	if(connection.start.index == connection.end.index) {
		direction = "all";
	} else if(connection.start.index == baseEntityIndexName) {
		direction = "out";
	} else {
		direction = "in";
	}
	...
	return (function() {
		if(options.runBefores) {
			return RunBeforeProcessing('read', null, options);
		} else {
			return Q.fcall(function() { return data; });
		}
	})().then(function(data) {
		...
		if(options.runAfters) {
			return RunAfterProcessing('after', resp, options);
		} else {
...
function relationshipDeleteHandler(req, res) {
	...
	__app.deleteRelationship(
		...
		graphReq.baseEntity.keyProperty,
		req.params.keyvalue,
		graphReq.connectedEntity.index,
		graphReq.connectedEntity.keyProperty,
...
function RunAfterProcessing(operation, response, graphReq) {
	return RunProcessing(false, operation, response, graphReq);
}

function RunBeforeProcessing(operation, response, graphReq) {
	return RunProcessing(true, operation, response, graphReq);
}

function RunProcessing(before, operation, response, graphReq) {
	var d = Q.defer();
	var processStore = (before ? __app.beforeProcessingFns : __app.afterProcessingFns);
	var path = graphReq.entityName;
