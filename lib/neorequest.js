...
NeoRequest.prototype.createIndexedNode = function(nodeData, indexName) {
	...
	return $this.createIndex(indexName).then(function() {
		...
		if(!nodeKeyValue) { //nodeKeyValue was not provided initially. Now that we have the ID of the new node, we will add that as the key.
			self = r.body.self;
...
NeoRequest.prototype.getEntity = function(indexName) { return _.find(this.app.entities, function(itm) { return itm.indexName == indexName; }); }
...
NeoRequest.prototype.createRelationship = function(startNodeIndex, startNodeKey, endNodeIndex, endNodeData, relationshipName, dir) {
	...
	return $this.getIndexedNode(startNodeKey, startNodeIndex).then(function(r) { //check if the start node exists
		...
		if(!existingRel) {
			...
			var targetEndpoint = (to ? startEntity.name + "." + endEntity.indexName : endEntity.name + "." + startEntity.indexName);
			try {
				$this.runRule("create", targetEndpoint, startNodeData, endNodeData, relPostObj.data);
			} catch(err) {
				return Q.fcall(function() { throw err; });
			}
			...
			if(relData && existingRel.properties) { //if relationship properties were included in the request		
				...
				var targetEndpoint = (to ? startEntity.name + "." + endEntity.indexName : endEntity.name + "." + startEntity.indexName);
				try {
					$this.runRule("update", targetEndpoint, startNodeData, endNodeData, relData);
				} catch(err) {
					return Q.fcall(function() { throw err; });
				}
...
NeoRequest.prototype.runRule = function(operation, path, startNodeData, endNodeData, relationshipData) {
	var rule = _.find(this.app.accessRules, function(itm) {
		return itm.operation == operation && itm.path == path;
	});
	if(rule) {
		return rule.func(this.requestInfo, startNodeData, endNodeData, relationshipData);
	}
}
