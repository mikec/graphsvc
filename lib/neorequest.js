...
NeoRequest.prototype.deleteIndexedNode = function(keyvalue, indexName) {
	...
	return $this.getIndexedNode(keyvalue, indexName).then(function(s) {
		...
		$this.clearBatchRequests();
		for(var i in r.body) {
			$this.addBatchRequest("DELETE", r.body[0].self);
		}
...
NeoRequest.prototype.getRelationships = function(nodeKeyValue, nodeIndex, relationshipType, to) {
	...
	return this.getIndexedNode(nodeKeyValue, nodeIndex).then(function(r) {
		...
		var inOrOut = (to ? "out" : "in");
		return request.get(nodeUrl + '/relationships/' + inOrOut + '/' + relationshipType);
	});
}

//creates a new relationship
NeoRequest.prototype.createRelationship = function(startNodeIndex, startNodeKey, endNodeIndex, endNodeData, relationshipName, to) {
	var $this = this;
	...
	return $this.getIndexedNode(startNodeKey, startNodeIndex).then(function(r) { //check if the start node exists
		...
		if(endNodeUrl) {
			return $this.getRelationships((to ? startNodeKey : keyval), (to ? startNodeIndex : endNodeIndex), relationshipName, true);
		} else {
		...
		if(!existingRel) {
			...
			return Q.fcall(function() { throw new Error("CREATE CONNECTION " + relDefString + " FAILED: Connection already exists") });
		}
	});
}
