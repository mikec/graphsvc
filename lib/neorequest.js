...
NeoRequest.prototype.updateIndexedNode = function(nodeData, indexName) {
	...
	return $this.getIndexedNode(keyvalue, indexName).then(function(s) {
		...
		if(nodeUrl) {
			...
			try {
				$this.runRule("update", entity.name, newNodeData);
			} catch(err) {
				return Q.fcall(function() { throw err; });
			}
...
NeoRequest.prototype.getRelatedNodes = function(nodeKeyValue, nodeIndex, relationshipType, dir) {
	...
	if(this.requestInfo && this.requestInfo.skip >= 0) this.skip = this.requestInfo.skip;
	if(this.requestInfo && this.requestInfo.limit >= 0) this.limit = this.requestInfo.limit;
	...
	return this.executeCypherQuery(countQuery, params).then(function(r) {
		...
		relNodes.data = nodesWithoutErrors;
...
NeoRequest.prototype.runRule = function(operation, path, startNodeData, endNodeData, relationshipData) {
	...
	for(var i in rules) {
		...
		if(startNodeData && !endNodeData) fnParams.nodeData = startNodeData;
		else if(startNodeData && endNodeData) {
			fnParams.startNodeData = startNodeData;
			fnParams.endNodeData = endNodeData;
		}
		if(relationshipData) fnParams = relationshipData;
		rule.func(fnParams);
	}
}
