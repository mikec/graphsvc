...
function RequestInfo(req) {
	...
	if(pathObj.connection) { // if the request is for a connection
		...
		this.connection.direction = (this.connection.start.index == this.index ? "out" : "in");
		if(this.connection.start.index == this.connection.end.index) this.connection.direction = "all";
