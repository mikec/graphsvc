...
function nodeGetHandler(req, res) {
	...
	var conns = _.filter(__app.connections, function(itm) {
		return (itm.start.index == pathObj.index) || (itm.end.index == pathObj.index);
	});
	...
	__app.neorequest.getIndexedNode(req.params.keyvalue, pathObj.index).then(function(s) {
		...
		for(var i in connections) {
			...
			nodeData.connections[cObj.urlName] = host + '/' + pathObj.index + '/' + req.params.keyvalue + '/' + cObj.urlName;
		...
		if(getRelatedNodeCallParams.length > 0) {
			...
			getRelatedNodeCallParams.forEach(function (connObj) {
				result = result.then(function() {
					return getRelatedNodes(req.params.keyvalue, pathObj.index, connObj.relationshipName, connObj.dir).then(function(relatedNodes) {
