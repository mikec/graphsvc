...
function relationshipGetHandler(req, res) {
	...
	__app.neorequest.getRelatedNodes(pathObj.key, pathObj.index, conn.relationshipName, dir).then(function(r) {
		...
		res.send(nodesWithRelData);
	}, function(err) {
		res.send(errorResp(err));
	}).done();
	
	/*
	//move to neorequest.getRelatedNodes()
	var nodeToRelationshipMap = {};
	var nodes = {};
	return (function() {
		if(dir == "all") { // if the requested relationship is between two nodes of the same type, get the start node first
			
		} else {
			return Q.fcall(function() { return true; });
		}
	}).then(function(r) {
		return __app.neorequest.getRelationships(pathObj.key, pathObj.index, conn.relationshipName, dir);
	}).then(function(r) {
		//get data for each node, and each set of relationship properties
		__app.neorequest.clearBatchRequests();
		for(var i in r.body) {
			var nodeUrl = (dir == "out") ? r.body[i].end : r.body[i].start);
